...
def main():
    ...
    for i, sample in enumerate(test_loader):
        ...
        for j, bbox in enumerate(sample['left_bboxes']):
            ...
            ### Median csv file
            ### ['No.', 'x_min', 'x_max', 'y_min', 'y_max','GT_disp', 'PredDisp', 'EPE', 'GT_depth', 'PredDepth', 'DepthErr']
            data_median = [j, x_min, x_max, y_min, y_max, float(np.median(gt_disp_np)), float(np.median(pred_disp_np)), float(epe_np), np.median(gt_depth), np.median(pred_depth), np.median(depth_err)]
            with open(csvFileName_median, 'a', newline='') as file:
                csvwriter_median = csv.writer(file)
                csvwriter_median.writerow(data_median)
            wandb.log({'id': j,'GTDispMedian': gt_disp.median(), 'PredDispMedian': pred_disp_bb.median(), 'EPE': epe, 'GTDepthMedian': np.median(gt_depth), 'PredDepthMedian': np.median(pred_depth), 'DepthErrMedian': np.median(depth_err)})
            ...
            for b in range(pred_disp.size(0)):
                ...
                # mask_pred = (pred_disp > 0) & (pred_disp < args.max_disp)
                # dist_error = dist_err((disp_pred * 256.).astype(np.uint16), (disp_gt* 256.).astype(np.uint16), mask_pred.detach().cpu().numpy())
